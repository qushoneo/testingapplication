...
export default function TestPlanRecovery({
  ...
  const testPlanId = Number(use(params).id);
  const projectId = Number(use(params).projectId);

  const router = useRouter();
  const { setSelectedProject } = useProjectStorageStore();

  if (!testPlanId || !projectId) {
    return <div>Invalid test plan id or project id</div>;
  }
  ...
  const { data: testPlan, isLoading: isTestPlanLoading } = useFetch(
    `projects/${projectId}/test_plans/${testPlanId}`
  );

  const { openStartTestRun, isStartTestRunOpen } = useModalStore();
    ...
    <ProtectedRoute className="px-[30px] pb-[20px] flex flex-col gap-[20px]">
      <div className="flex flex-col gap-[20px] sticky top-[0px] pt-[20px] z-[10] bg-white">
        <div className="w-full h-[40px] flex items-center gap-[8px]">
          ...
          <Button
            label="Start run"
            className="w-[130px] mt-[4px] ml-auto"
            variant="filled"
            icon="play"
            onClick={() => openStartTestRun(testPlanId)}
          />
        </div>
      ...
      {isStartTestRunOpen && (
        <StartTestRunModal projectId={testPlan.projectId} />
      )}
    </ProtectedRoute>
  );
}
